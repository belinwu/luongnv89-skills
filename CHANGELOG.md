...
### Bug Fixes
- **install.sh / remote-install.sh**: Suite-aware copy. The previous `cp -r "$src"/* "$dest"/` recursed into nested phase children (e.g., `skills/website-cloner/website-analyzer/`), so installing the `website-cloner` umbrella duplicated all six children inside its destination — and selecting the umbrella plus a child installed the child twice. Both installers now route through a `copy_skill_files` helper that skips any subdirectory containing its own `SKILL.md`. Each child remains independently installable via `discover_skills`. (#43)

### Features
- **clean-code**: New skill at **1.2.0** — user-invoked audit of a codebase against the bbv Clean Code Cheat Sheet V2.2 (Urs Enzler), covering both halves of the source: Clean Code principles (SOLID, code smells, naming, methods, conditionals, exceptions, dependencies, design) and Clean ATDD/TDD practices (test smells, design-for-testability, TDD process smells, red/green patterns, test pyramid, CI). Runs a five-phase inline workflow — scope → audit against the two `references/` checklists → synthesize & rank by severity → build a phased plan → write & verify — and produces `CLEAN_CODE_AUDIT.md` with a severity summary, findings grouped by category (each citing `file:line` and a named principle/smell), and a priority-phased implementation plan (Phase 1 Critical → Phase 2 Major → Phase 3 Minor) where every task carries an ID, target `file:line`, principle, effort estimate, dependencies, and acceptance check. Plan-only by design: it never edits source code. **1.2.0** adds an optional, self-contained offline-safe HTML report (`CLEAN_CODE_AUDIT.html`) — a refined dark-IDE template (`references/report-template.html` + `references/html-report-guide.md`) with metric tiles, a CSS severity donut, effort-by-phase bars, collapsible findings, and an interactive phased plan with localStorage progress tracking (no CDN, no chart library). The description leads with the user-invoked gate so it survives `/skills`-listing truncation; doesn't auto-trigger on general coding, bug-fixing, or PR review — those route to `code-review`. Cleared the skill-creator standard (`quick_validate` clean, frontmatter audit, body <500 lines) and the asm-eval 85/8 floor (overall 94, grade A). Not for general coding, bug-fixing, performance tuning, or ordinary PR review.
- **viral-product-evaluator**: New skill at **1.1.0** — grades a product's codebase + landing page against the 32 viral-product principles into a Virality Score (/100) and a prioritized, impact-ordered fix list. Inputs: a live URL (fetched headlessly), a local HTML/JSX/MDX file, or auto-detected page from the repo, plus the codebase for the pricing/paywall/subscription, feature-surface, and "does one thing" checks. Each principle scores PASS/PARTIAL/FAIL with product-specific evidence; visual/subjective principles (OG-image punch, founder presence, emotional headline, novelty) are flagged low-confidence. The rubric, report template, and step-report formats live in `references/`. The 1.1.0 patch hardens scoring to a deterministic mechanical tally (no hand-counting) and treats a single fixed-price SKU as a clean PASS on Popcorn Pricing.
- **tmux-agent-comms**: Robust send → wait → read loop (1.2.0 → 1.3.0). Phase 3 adds a bounded (~5s) delivery-verification step after send and before waiting, keyed off post-send activity (spinner / `esc to interrupt`) rather than the message text being on screen, so a dropped keystroke or unsubmitted `Enter` is detected and reported distinctly from a reply timeout. Phase 4 treats `wait_for_idle.py`'s verdict as advisory, adds an independent manual capture-pane read to tell a stalled agent from a working one, and enforces a hard overall budget on the re-wait/re-send loop with escalation to the user (no infinite polling). Phase 5 makes a bounded-tail read (~20–40 lines, `-S -40`) the default for capturing a full reply, with truncation detection and stepwise expansion, distinct from unbounded scrollback. Threaded through the Example, Edge Cases, Step Completion Report, and the references recipes/troubleshooting table. (#52, closes #49/#50/#51)
- **tmux-agent-comms**: Quality pass to clear the asm-eval 85/8 floor (1.1.0 → 1.2.0, overall 86 → 93, grade B → A). Added `## When to Use`, `## Workflow` (quick-start orientation), `## Example` (a real send → wait → read interaction with an `Expected output:` block), and `## Edge Cases` (trust-dialog exit 3, numbered-list non-block, duplicate-session, busy-prose) — all drawn from the eval run, no keyword filler. prompt-engineering 5 → 9, testability 7 → 10; quick_validate stays clean, body 201 lines.
- **tmux-agent-comms**: Reliability + token pass (1.0.0 → 1.1.0). `wait_for_idle.py` now returns three states by exit code — `0` idle, `3` blocked on a needs-human prompt (trust/auth dialog), `2` timeout — so the skill never fires a message into a dialog where it would be read as menu input. Busy markers stay narrow (2-line scan of unambiguous spinner chrome like `esc to interrupt`); block markers use a wider 12-line scan for multi-line dialog boxes, and both use only VERIFIED strings (observed on real Claude Code 2.x / Gemini CLI boots) — generic prose like "running" or "1. yes" is deliberately excluded to avoid false positives, with `--busy-marker`/`--block-marker` and `TAC_*` env vars for other agents. Default output is the reply **delta** (new lines since the wait started, ~4 lines vs a 24-line full pane in testing), the dominant token saving over multi-turn conversations; `--full` restores whole-pane output. New `broadcast.sh` fans one message to a fleet and waits on all sessions concurrently (wall-clock = slowest agent, not the sum), emitting one labeled per-agent block. Spawn path now checks-or-uniquifies the session name before `new-session` (which fails on duplicates) and treats "ready" as "input box up / not blocked," not "process started." Description broadened to "any CLI agent" to match the agent-agnostic stability signal.
- **release-manager**: Add a fourth parallel subagent, `landing-page-updater`, to the release fan-out (2.4.1 → 2.5.0). When the project ships a landing page (a marketing/home web page, detected via `index.html` under `public/`/`static/`/`www/`/etc., framework home pages, or deploy hints like `vercel.json`/`netlify.toml`/`CNAME`/Pages workflows), it proposes release-narrative updates: version display, download/install CTA, "What's New", and feature highlights. Libraries/CLIs with no landing page get a clean no-op (`landing_page_found: false`). To keep two agents from editing the same line, the new agent stays in its lane — release narrative only — and defers raw version-string bumps to the `version-bumper`, listing any unavoidable overlap under `version_overlap`. `release-reviewer` now reads the landing report and flags cross-agent same-file/line collisions as high severity; the documented apply order is version → changelog → docs → landing. Description left unchanged (internal step, not a trigger phrase).
- **security-setup**: Add optional Socket Firewall supply-chain install guardrail guidance (1.3.1 -> 1.3.2). For macOS/Linux `zsh`/`bash`, the skill now recommends `sfw` aliases for the package managers a project actually uses (`npm`, `yarn`, `pnpm`, `pip`, `uv`, `cargo`) so everyday installs flow through Socket Firewall by default. This is documented as developer shell setup, not a required pre-commit dependency, and complements lockfile scanning instead of replacing it.
- **website-analyzer**: New skill — 6-dimension website analysis (UI/UX, category, style, performance, surface-level security, SEO) producing structured JSON for downstream consumers in the `website-cloner-suite`. Performance covers LCP/CLS/TTFB estimates, total page weight, and request count. SEO yields a 0–100 score with weighted per-dimension breakdown (meta tags, headings, alt text, structured data, crawlability). Handles unreachable, SPA, paywall, redirect-loop, and empty-page failure modes with structured error output. (#29)
- **website-analyzer**: Explicit per-dimension SEO sub-scoring rubrics (1.0.0 → 1.0.1). Each of the five SEO dimensions now ships a deterministic 0–100 rubric (title/description length thresholds, h1/h2 structure rules, alt-text coverage ratio, JSON-LD vs. OG/microdata fallback, crawlability points for canonical + robots.txt + sitemap), plus a redistribution rule for unverifiable dimensions. Reduces output drift across runs. README catalog entries added for `website-analyzer` and `website-cloner`. (#36)
